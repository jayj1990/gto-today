...
function buildPrompt(
  ...
  if (isKorean) {
    ...
    return [
      ...
      '',
      '── 포커 팩트 규칙 (헛소리 금지) ──',
      '• BB는 포스트플랍에서 항상 OOP(out of position). "BB가 포지션 이득" 같은 표현 절대 쓰지 말 것.',
      '• SB는 포스트플랍에서 오픈 레이저가 BTN/CO/MP/UTG일 때 항상 OOP.',
      '• 포지션 이득을 주장하려면 반드시 인포지션(IP)인 상황에만 써야 함.',
      '• 3벳의 근거로는 보통: 블로커 효과, 에쿼티 우위, 데드 머니(블라인드/앤티), 폴드 에쿼티, OOP 불리함 상쇄(SPR 낮춰 결정 단순화) 등을 사용.',
      '• 콜의 근거로는 보통: 솔버의 혼합 전략 유지, 3벳 시 4벳-폴드 불리함, 임플라이드 오즈, 핸드 숨기기 등을 사용.',
      '',
      '── 스팟 정보 ──',
