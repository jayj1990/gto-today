...
function buildPrompt(
  ...
  if (isKorean) {
    ...
    return [
      ...
      '',
      '── 출력 형식 ──',
      '아래 3가지를 각각 한두 문장으로 이어지는 글로 써주세요. 번호나 불릿 없이 자연스러운 세 문단으로.',
      '1) 이 핸드가 왜 이런 빈도로 플레이되는지',
      '2) 함께 고려해야 할 것들 (레인지 어드밴티지, 포지션, 블로커, 플레이어빌리티 등)',
      '3) 처음 배우는 사람이 이 스팟에서 자주 하는 실수',
      '',
      `어투: ${toneGuidance}`,
      '전체 길이: 한국어 200자 이내. 마크다운 문법(헤더/볼드) 금지, 평범한 완결 문장으로.',
