    ...
    <main className="mx-auto max-w-5xl safe-pad-x pb-24 pt-10">
      <header className="mb-8">
        ...
        <p className="mt-2 text-fg-muted">
          같은 로고를 헤더 · 스플래시 · 앱 아이콘 세 자리에 넣어서 실제 사용 상황에서 어떻게 보이는지 비교합니다.
        </p>
      </header>
      ...
      <section className="mt-16 rounded-[var(--radius-panel)] border-hair surface p-6">
    ...
    <div className="space-y-6">
      ...
      <Mock label="스플래시 (140px, 투명 배경 · 다크 그라디언트 위)">
        <div
          ...
          style={{
            background:
              'radial-gradient(circle at 50% 40%, #0E3B2E 0%, #082018 65%, #051612 100%)',
          }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
        ...
        <div className="flex items-end justify-around gap-4 p-6" style={{ background: '#08120E' }}>
          {[180, 96, 48, 24].map((size) => (
            <div key={size} className="flex flex-col items-center gap-2">
              ...
              />
              <span className="font-mono text-[10px] text-fg-muted">{size}px</span>
            </div>
          ))}
        </div>
      </Mock>
...
function Mock({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <figure className="overflow-hidden rounded-[var(--radius-panel)] border-hair">
      <figcaption className="px-3 py-2 font-mono text-[10px] uppercase tracking-[0.2em] text-fg-muted">
        {label}
      </figcaption>
