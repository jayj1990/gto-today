...
async function listAssets() {
  ...
  try {
    ...
    for (const cat of categories) {
      ...
      const files = (await readdir(catPath)).filter((f) => f.endsWith('.png')).sort();
      ...
      <main className="safe-pad-x mx-auto max-w-5xl pb-24 pt-10">
        <header className="mb-8">
          ...
          <p className="text-fg-muted mt-2">
            DALL·E 3로 생성된 후보들. 카테고리별로 나열됐어요. 마음에 드는 것 말씀 주시면 해당
            파일을 최종 자리 ({`/public/logos/, /public/onboarding/ 등`})로 이관하고 PNG 파생본
            생성합니다.
          </p>
        </header>

        {groups.length === 0 && (
          <div className="border-hair surface rounded-[var(--radius-panel)] p-8 text-center">
            <p className="text-fg-muted font-mono text-[13px]">
        ...
        <ul className="space-y-10">
          {groups.map(({ category, files }) => (
            <li key={category}>
              <h2 className="text-fg-muted font-mono text-[12px] uppercase tracking-[0.22em]">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                {files.map((file) => {
                  ...
                  return (
                    <figure
                      key={file}
                      className="border-hair surface overflow-hidden rounded-[var(--radius-panel)]"
                    >
                      <div
                      ...
                      <figcaption className="p-3">
                        <p className="text-fg font-mono text-[11px]">{label}</p>
                        <p className="text-fg-muted mt-1 font-mono text-[10px]">{src}</p>
                      </figcaption>
                    </figure>
                  );
                })}
